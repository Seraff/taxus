 ...
 */
    ...
    phylotree.hide_context_menu = function(){
      var menu_object = d3
        .select(self.container)
        .select("#" + d3_layout_phylotree_context_menu_id);
      menu_object.style("display", "none");
 ...
 * @param {Boolean} safe (Optional) don't reassign zoom event.
 * @returns The current ``phylotree``.
 */
    ...
    phylotree.safe_update = function(transitions){
      var old_transform = d3.select("."+css_classes["tree-container"]).attr('transform');
      phylotree.update(transitions, true);
      d3.select("."+css_classes["tree-container"]).attr('transform', old_transform);
 ...
 */
    ...
    phylotree.draw_node = function(container, node, transitions) {
      ...
      container.on("mousemove", function(c){
        dist=Math.abs(d3.event.movementX)+Math.abs(d3.event.movementY);
        if (dist > 0) {
      ...
      container.on("mouseup", function(){
        if (!container.dragged) {
          phylotree.modify_selection([node], selection_attribute_name);
        }
      });
      ...
      if (d3_phylotree_is_leafnode(node)) {
        ...
        labels
          .on(options["context-menu-event"], function(d, i) {
        ...
        if (radius > 0) {
          ...
          circles
            ...
            .on(options["context-menu-event"], function(d) {
              phylotree.handle_node_click(d);
            });
        } else {
          circles.remove();
 ...
 */
    ...
    phylotree.get_svg = function(){ return svg }
    phylotree.get_offsets = function(){ return offsets }
